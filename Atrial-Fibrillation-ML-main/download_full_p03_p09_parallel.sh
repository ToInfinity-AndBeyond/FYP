...
LOG_DIR="${SCRIPT_DIR}/artifacts/mimic_ext_subject_selection/full_download_logs_parallel"
...
for prefix in "${PREFIXES[@]}"; do
  ...
  while true; do
    ...
    echo
    echo "=== Starting ${prefix} (attempt ${attempt}, jobs=${JOBS}) ==="
    echo "manifest=${manifest}"
    echo "log=${log_path}"
    set +e
    JOBS="$JOBS" "${SCRIPT_DIR}/download_selected_mimic_ext_parallel.sh" "$manifest" | tee "$log_path"
    status=${PIPESTATUS[0]}
    set -e
    if [[ $status -eq 0 ]]; then
